...
public static class CC_INPUT {
    ...
    private class ButtonState {
        ...
        public void UpdateButtonState(Wand wand, WandButton button) {
            ...
            if (CC_CONFIG.IsDestiny()) {
                ...
                if (wand == Wand.Left)
                    wandString = "CC_FLAT_WAND0@";
                else if (wand == Wand.Right)
                    wandString = "CC_FLAT_WAND1@";

                int vrpnChannel = getViveVRPNChannel(button);
                bool currentValue = VRPN.vrpnButton(wandString + CC_CONFIG.innovatorIP, vrpnChannel);
    ...
    private class AxisState {
        ...
        public void UpdateAxisState(Wand wand, WandAxis axis) {
            ...
            if (CC_CONFIG.IsDestiny()) {
                ...
                if (canoe.isVive()) {
                    int vrpnChannel = getViveVRPNChannel(axis);

                    if (wand == Wand.Left) {
                        axisValue = (float)VRPN.vrpnAnalog("CC_FLAT_WAND0@" + CC_CONFIG.innovatorIP, vrpnChannel);
                    } else if (wand == Wand.Right) {
                        axisValue = (float)VRPN.vrpnAnalog("CC_FLAT_WAND1@" + CC_CONFIG.innovatorIP, vrpnChannel);
                    } else {
                        axisValue = 0.0f;
                    }
                } else if (canoe.isOptiTrack()) {
                    int vrpnChannel = getOptiTrackVRPNChannel(axis);
    ...
    /// <summary>
    /// Returns VRPN channel for Vive button input.
    /// </summary>
    /// <param name="button">Wand button. </param>
    private static int getViveVRPNChannel(WandButton button) {
        switch (button) {
            case WandButton.Menu:
                return 1;
            case WandButton.Grip:
    ...
    /// <summary>
    /// Get the corresponding Vive VRPN channel for the axis.
    /// </summary>
    /// <param name="axis">Wand axis (x-axis, y-axis, trigger).</param>
    private static int getViveVRPNChannel(WandAxis axis) {
        switch (axis) {
            case WandAxis.XAxis:
                return 0;
            case WandAxis.YAxis:
        ...
        throw new System.ArgumentException("Vive axis does not exist.");
    }

    /// <summary>
    /// Get the corresponding OptiTrack VRPN channel for the axis.
    /// </summary>
    /// <param name="axis">Wand axis (x-axis, y-axis, trigger).</param>
    private static int getOptiTrackVRPNChannel(WandAxis axis) {
        switch (axis) {
            case WandAxis.XAxis:
                return 0;
            case WandAxis.YAxis:
        ...
        throw new System.ArgumentException("OptiTrack axis does not exist.");
    }
